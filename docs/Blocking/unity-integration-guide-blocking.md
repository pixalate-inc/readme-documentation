---
title: "Unity Integration Guide (Blocking)"
excerpt: "The Pixalate Pre-Bid Blocking SDK provides easy an easy implementation of Pixalate's Pre-Bid Fraud Blocking API in your Unity application."
deprecated: false
hidden: false
metadata:
  title: ''
...
Download the latest .unitypackage file from the [releases section](https://github.com/pixalate-inc/unity-prebid-blocking/releases) of the repository. Double-click it while your project is open to import the files into your project. They will automatically be extracted into your project.
...
---|---|---  
blockingThreshold |  The probability threshold at which blocking should occur. Normal range is anywhere from 0.75-0.9. |  0.75  
...
The SDK supports multiple logging levels which can provide additional context when debugging. The current level can be set through Pixalate.setLogLevel, and defaults to LogLevel.Info. Logging can be disabled entirely by setting the level to LogLevel.None.
...
When extending the DefaultBlockingStrategy, make sure to override the -Impl variety methods rather than the interface methods so as to preserve caching behavior.
    ...
        public override void GetIPv4Impl (CancellationToken token, Action callback) {  
            callback.done( null );  
        }  
    }  
...
To create a custom strategy from scratch, you must extend the `BlockingStrategy` interface. All of the methods have default implementations returning null, meaning you only need to override the strategies you want to provide values for.
    ...
        public async void GetIPv4 (CancellationToken token, Action callback) {  
    ...
        public void GetDeviceID (CancellationToken token, Action callback) {  
          callback( null );  
        }  
      
        public void GetUserAgent (CancellationToken token, Action callback) {  
          callback( null );  
        }  
    }
